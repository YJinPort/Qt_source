...
//         combo->addItem("http://news.google.co.kr/news?cf=all&hl=ko&output=rss");
    ...
    for(int i = 0; i < urlList.count(); i++) combo->addItem(urlList.at(i));
    if(lib.isLoaded()) lib.unload();

    if(urlList.count() == 0) combo->addItem("http://rss.joins.com/joins_news_list.xml");
...
//    settings.endArray( );
    ...
    SaveUrls* saveUrl = (SaveUrls*)lib.resolve("SaveUrls");
    QStringList urlList;
    for(int i = 0; i < combo->count(); i++) urlList.append(combo->itemText(i));
